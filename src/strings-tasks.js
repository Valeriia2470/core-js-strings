...
function removeFirstOccurrences(str, value) {
  ...
  return str.substring(0, first) + str.substring(first + value.length);
}
...
function removeLastOccurrences(str, value) {
  ...
  return str.substring(0, last) + str.substring(last + value.length);
}
 ...
 *   isPletalindrome('No lemon, no melon') => true
 */
function isPalindrome(str) {
  const low = str.toLocaleLowerCase().replace(/[^a-z0-9]/g, '');
  if (str === '') return true;
...
function invertCase(str) {
  return str
    .split('')
    .map((chara) =>
      chara === chara.toLowerCase() ? chara.toUpperCase() : chara.toLowerCase()
    )
    .join('');
}
...
function extractEmails(str) {
  const email = str.split(';').map((ema) => ema.trim());
  return email;
}
...
function encodeToRot13(str) {
  return str
    .split('')
    .map((char) => {
      const cod = char.charCodeAt(0);
      if (cod >= 97 && cod <= 122) {
        return String.fromCharCode(((cod - 97 + 13) % 26) + 97);
      }
      if (cod >= 65 && cod <= 90) {
        return String.fromCharCode(((cod - 65 + 13) % 26) + 65);
      }
      return char;
    })
    .join('');
...
function getCardId(value) {
  ...
  const ran = value.slice(0, -1);
  const mas = value.slice(-1);
  const ranI = ranks.indexOf(ran);
  const masI = mast.indexOf(mas);
  if (ranI === -1 || masI === -1) {
    return '0';
  }
  return masI * 13 + ranI;
